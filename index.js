const fullText =
    `
2f0cdfe464344e8650edc59daac3504b1710d56b89dce5011e8c90f6`.split("\n");



let a = "280dc9e47f3352c307f6d894ee8d534313429a79c1d8a6021f8a8eabca919cfb685a0d468973625e757490daa981ea6b";
a = Buffer.from(a, "hex");
function xorHexes(a, b) {
    const res = [];

    for (const [index, el] of a.entries()) {
        res.push(el ^ (b[index] ?? 0));
    }
    return Buffer.from(res);
}

for (let i =0; i<fullText.length; i++)
{
    const b = Buffer.from(fullText[i], "hex");
    const pseudoKey = xorHexes(a, b);
    console.log([
        xorHexes(Buffer.from("For who would bear the wh"), pseudoKey).toString(),
    ]);

}
// [ 'For who would bear the wh' ]
// [ "Th'oppressor's wrong, the" ]
// [ "The pangs of dispriz'd lo" ]
// [ 'The insolence of office, ' ]
// [ "That patient merit of th'" ]
// [ 'When he himself might his' ]
// [ 'With a bare bodkin? Who w' ]
// [ 'To grunt and sweat under ' ]
// [ 'But that the dread of som' ]
// [ "The undiscovere'd country" ]
// [ 'No traveller returns, puz' ]
// [ 'And makes us rather bear ' ]
// [ 'Than fly to others that w' ]
// [ 'Thus conscience doth make' ]
// [ 'And thus the native hue o' ]
// [ "Is sicklied o'er with the" ]
// [ 'And enterprises of great ' ]
// [ 'With this regard their cu' ]
// [ 'And lose the name of acti' ]

